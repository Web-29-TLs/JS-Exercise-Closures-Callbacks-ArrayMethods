 ...
 * should return 1000.
 */
function processSum(numArr, cb) {
  const reduced = numArr.reduce((acc, curr) => acc + curr, 0);
  return cb(reduced);
}
...
function processContains(item, arr, cb) {
  return arr.includes(item) ? cb(true) : cb(false);
}
...
function getFullNames(arr) {
  let runnersByFullName = [];
  arr.forEach(runner =>
    runnersByFullName.push(`${runner.last_name}, ${runner.first_name}`)
  );
  return runnersByFullName;
}
...
function firstNamesAllCaps(arr) {
  let runnersFirstNameInCaps = [];
  arr.forEach(runner =>
    runnersFirstNameInCaps.push(`${runner.first_name.toUpperCase()}`)
  );
  return runnersFirstNameInCaps;
}
...
function getRunnersByTShirtSize(arr, size) {
  const runnersByShirtSize = [];
  arr.map(
    runner => runner.shirt_size === size && runnersByShirtSize.push(runner)
  );
...
function tallyUpDonations(arr) {
  let donations = [];
  arr.map(amount => donations.push(amount.donation));
